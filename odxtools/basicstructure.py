...
class BasicStructure(DopBase):
    ...
    def _message_format_lines(self, allow_unknown_lengths: bool = False) -> List[str]:
        ...
        def param_sort_key(param: Parameter) -> Tuple[int, int, int]:
            ...
            param_bit_length = param.get_static_bit_length() if param.get_static_bit_length() is not None else 0
            ...
            return (byte_position_int, 1000000 - param_bit_length, 8 - bit_position_int)
        ...
        while param_idx <= len(params) and not error:  # For each byte
            ...
            if (param_idx < len(params)
                    and hasattr(params[param_idx], 'byte_position')
                    and params[param_idx].byte_position == byte_idx):
